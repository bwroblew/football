...
r"""Player from HER cnn checkpoint.

Example usage with play_game script:
python3 -m gfootball.play_game \
    --players "her_cnn:left_players=1,checkpoint=$YOUR_PATH,policy=$POLICY"
...
class Player(player_base.PlayerBase):
  """An agent loaded from HER cnn model checkpoint."""
  ...
  def __init__(self, player_config, env_config):
    ...
    with tf.variable_scope(self._player_prefix):
      with tf.variable_scope('her_model'):
